...
using GetStateChangedFieldDelegate = System.Func<object, Flux.IStateChangedNotifier>;

namespace Flux
{
	public static class StateSubscriber
	{
		private static readonly ConcurrentDictionary<Type, IEnumerable<GetStateChangedFieldDelegate>>
			ValueDelegatesByType;
		...
		public static IDisposable Subscribe(object subject, Action<IStateChangedNotifier> callback)
		{
			...
			foreach (GetStateChangedFieldDelegate getStateChangedNotifierFieldValue in
			         getStateChangedNotifierFieldDelegates)
			{
			...
			return ValueDelegatesByType.GetOrAdd(type, _ =>
			{
				...
				foreach (FieldInfo currentField in stateChangedNotifierFields)
				{
					var getValueDelegate =
						new GetStateChangedFieldDelegate(x => (IStateChangedNotifier)currentField.GetValue(x)!);
